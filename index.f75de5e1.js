...
function generateNewNumber() {
    ...
    for(let row = 0; row < 4; row++){
        for(let col = 0; col < 4; col++)if (gameField[row][col] === 0) emptyCells.push({
            row,
            col
        });
    }
    ...
    for(let i = 0; i < fieldCells.length; i++){
        ...
        const col = i % 4;
        fieldCells[i].textContent = gameField[row][col] === 0 ? "" : gameField[row][col];
        fieldCells[i].className = "field-cell";
        if (gameField[row][col] !== 0) fieldCells[i].classList.add(`field-cell--${gameField[row][col]}`);
    }
    gameScore.textContent = score;
}
function moveUp() {
    ...
    for(let col = 0; col < 4; col++){
        for(let row = 1; row < 4; row++)if (gameField[row][col] !== 0) {
            let currentRow = row;
...
function moveDown() {
    let moved = false;
    for(let col = 0; col < 4; col++){
        for(let row = 2; row >= 0; row--)if (gameField[row][col] !== 0) {
            let currentRow = row;
...
function moveRight() {
    let moved = false;
    for(let row = 0; row < 4; row++){
        for(let col = 2; col >= 0; col--)if (gameField[row][col] !== 0) {
            let currentCol = col;
...
function moveLeft() {
    let moved = false;
    for(let row = 0; row < 4; row++){
        for(let col = 1; col <= 3; col++)if (gameField[row][col] !== 0) {
            let currentCol = col;
